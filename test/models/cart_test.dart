...
void main() {
  group('Cart', () {
    ...
    test('remove decreases quantity and removes item when quantity reaches zero', () {
      final cart = Cart();
      final s = Sandwich(type: SandwichType.veggieDelight, isFootlong: true, breadType: BreadType.white);
    ...
    test('calculatePrice uses provided PricingRepository and sums per-item totals', () {
    ...
    test('items returns unmodifiable view', () {
      ...
      expect(() => cart.items.add(CartItem(sandwich: s, quantity: 1)), throwsA(isA<UnsupportedError>()));
    });
  });
}
